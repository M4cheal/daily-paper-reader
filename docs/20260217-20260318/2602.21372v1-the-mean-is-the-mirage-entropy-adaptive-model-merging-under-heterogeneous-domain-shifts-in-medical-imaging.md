...
title_zh: 均值即幻象：医学影像中异构领域偏移下的熵自适应模型合并
...
tags: ["query:tta"]
score: 8.0
evidence: 测试时间分布偏移下的全在线模型合并
tldr: 针对医疗影像中异构领域偏移导致传统平均合并策略失效的问题，本文提出一种熵自适应的在线模型合并方法。该方法通过测试时的前向传播，利用无标签批次数据的熵信息动态调整合并系数，并解耦编码器与分类头以解决不匹配问题。在多个医疗和自然图像数据集上的实验证明，该方法在保持单模型推理效率的同时，显著提升了模型在未知领域下的泛化性能。
selection_source: fresh_fetch
motivation: 传统的平均合并策略在面对医疗影像中复杂的异构领域偏移时表现不佳，且无法利用测试时的目标域信息进行实时调整。
method: 提出一种基于熵自适应的完全在线合并方法，通过解耦编码器和分类头并利用测试批次的熵动态计算合并系数。
result: 在九个医疗及自然图像分类数据集上的实验表明，该方法在多种骨干网络下均显著优于现有的先进基准模型。
conclusion: 熵自适应的在线合并策略能有效应对测试时的分布偏移，是提升医疗影像模型泛化能力且兼顾推理效率的有效手段。
---

## 摘要
在未知的测试时分布偏移下，模型合并往往会使朴素策略（如均值平均）变得不可靠。这一挑战在医学影像领域尤为突出，因为模型通常在诊所利用私有数据进行本地微调，从而产生因扫描仪、协议和人群而异的领域特定模型。当部署在未知的临床站点时，测试用例以无标签、非独立同分布（non-i.i.d.）的批次到达，模型必须在没有标签的情况下立即进行自适应。在这项工作中，我们提出了一种熵自适应的全在线模型合并方法，该方法仅通过前向传递即可生成特定于批次的合并模型，从而有效地利用了目标信息。我们进一步论证了为什么均值合并在异构领域偏移下容易失败且存在失配。接着，我们通过解耦编码器和分类头，并使用独立的合并系数进行合并，从而缓解了编码器与分类器之间的不匹配。我们在九个医学和自然领域泛化图像分类数据集上，使用两种骨干网络对我们的方法与最先进的基准方法进行了广泛评估，结果显示在标准评估和挑战性场景中均获得了一致的性能提升。这些性能提升是在保留测试时单模型推理的情况下实现的，从而证明了我们方法的有效性。
