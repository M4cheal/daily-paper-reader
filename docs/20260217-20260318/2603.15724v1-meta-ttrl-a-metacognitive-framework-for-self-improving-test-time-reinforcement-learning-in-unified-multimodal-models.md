...
## 摘要
现有的用于文本生成图像 (T2I) 的统一多模态模型 (UMMs) 测试时扩展 (TTS) 方法主要依赖于搜索或采样策略，这些策略仅产生实例级的改进，限制了从先前推理中学习以及在相似提示词间积累知识的能力。为了克服这些局限性，我们提出了 Meta-TTRL，一种元认知测试时强化学习框架。Meta-TTRL 在由 UMMs 元知识衍生的模型内在监控信号指导下进行测试时参数优化，实现了测试时的自我改进和能力级提升。大量实验表明，Meta-TTRL 在包括 Janus-Pro-7B、BAGEL 和 Qwen-Image 在内的三个代表性 UMMs 上具有良好的泛化性，在有限数据下，在组合推理任务和多个 T2I 基准测试中取得了显著提升。我们提供了首次全面分析，以研究测试时强化学习 (TTRL) 在 UMMs 的 T2I 生成中的潜力。我们的分析进一步揭示了有效 TTRL 背后的一个关键见解：元认知协同 (metacognitive synergy)，即监控信号与模型的优化机制对齐，从而实现自我改进。
