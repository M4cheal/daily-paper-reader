...
## 摘要
由于基于学习的机器人控制器通常是在线外训练并以固定参数部署的，它们在运行过程中应对不可预见变化的能力有限。受生物启发，本研究提出了一个在线持续强化学习框架，能够在部署期间实现自动适应。该方法基于模型强化学习算法 DreamerV3，利用世界模型预测残差来检测分布外（out-of-distribution）事件并自动触发微调。适应进度通过任务级性能信号和内部训练指标进行监控，从而能够在无需外部监督和领域知识的情况下评估收敛性。该方法在多种当代连续控制问题上得到了验证，包括高保真模拟中的四足机器人和现实世界中的模型车辆。文中展示并讨论了相关指标及其解释，并描述了由此产生的权衡。结果勾勒出自主机器人智能体如何能够超越静态训练模式，转向在运行期间具备自我反思和自我改进能力的自适应系统，就像它们的生物原型一样。
