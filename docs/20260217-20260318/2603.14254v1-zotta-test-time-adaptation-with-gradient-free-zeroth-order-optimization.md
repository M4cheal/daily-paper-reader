...
## 摘要
测试时自适应 (TTA) 旨在通过对无标签测试数据进行自适应，提高模型在分布偏移下的鲁棒性，但大多数现有方法依赖于反向传播 (BP)，这在计算上成本高昂，且与量化模型等不可微模型不兼容，限制了在众多边缘设备上的实际部署。最近的无 BP 方法减轻了开销，但要么特定于架构，要么在处理高维模型时的优化能力有限。我们提出了 ZOTTA，这是一个完全无 BP 的 TTA 框架，它通过零阶优化 (ZOO) 仅使用前向传递执行高效自适应。虽然 ZOO 在理论上具有吸引力，但由于缺乏标签，在处理高维参数空间时，朴素应用会导致收敛缓慢且优化不稳定。ZOTTA 通过以下方式克服了这些挑战：1) 分布鲁棒层选择，自动识别并冻结已经提取分布不变特征的层，仅更新领域敏感层，以降低优化维度并加速收敛；2) 空间特征聚合对齐，通过对齐源域和目标域之间全局聚合的空间特征来稳定 ZOO，从而减少梯度方差。这些组件共同实现了与架构无关且稳定的无 BP 自适应。在 ImageNet-C/R/Sketch/A 上的广泛实验表明，ZOTTA 的性能优于或相当于基于 BP 的方法，例如，在 ImageNet-C 上，与 SAR 相比，它减少了 84% 的内存使用量，并将准确率提高了 3.9%。
