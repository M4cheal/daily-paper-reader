...
tags: ["query:tta"]
score: 10.0
evidence: 稳定高维模拟的测试时间自适应
tldr: "针对工程模拟中机器学习代理模型在面对分布偏移时性能下降的问题，本文提出了一种基于D-最优统计量的测试时自适应（TTA）框架。该方法通过存储最具信息量的统计数据，解决了高维、非结构化回归任务中TTA不稳定的难题。在SIMSHIFT和EngiBench基准测试中，该方法以极低的计算成本实现了高达7%的分布外性能提升，是首个针对高维模拟回归和生成式设计优化的系统性TTA方案。"
selection_source: fresh_fetch
motivation: 现有的测试时自适应方法在处理高维、非结构化且具有复杂输入输出关系的模拟回归问题时表现不稳定。
method: 提出一种基于存储D-最优统计量的TTA框架，用于在测试阶段实现稳定的模型自适应和原则性的参数选择。
result: "在多个基准测试中，该方法在几乎不增加计算开销的情况下，将分布外预测性能提升了高达7%。"
conclusion: 本研究首次系统性地证明了TTA在高维模拟回归和生成式设计优化中的有效性，为提高代理模型的鲁棒性提供了新思路。
---
