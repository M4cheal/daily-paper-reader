...
tags: ["query:tta"]
score: 8.0
evidence: 用于连续任务中VLA模型的扩展LoRA专家
tldr: 针对视觉-语言-动作（VLA）模型在多任务学习中面临的任务干扰和存储开销挑战，本文提出CORAL框架。该方法冻结预训练骨干网络，为每个任务分配独立的轻量化LoRA专家，并通过动态推理引擎实现零开销的任务路由。CORAL通过参数隔离彻底消除了负迁移和灾难性遗忘，在真实机器人和多个仿真基准上表现优于联合训练，为可扩展的持续机器人学习提供了高效方案。
selection_source: fresh_fetch
motivation: 旨在解决多任务机器人学习中因梯度冲突导致的性能下降，以及为每个任务维护完整模型带来的存储和部署难题。
method: 通过冻结VLA骨干网络并为每个任务挂载独立的LoRA专家，利用动态管理引擎根据指令实时切换专家以实现参数隔离。
result: 在Galaxea R1真实机器人及LIBERO等三个仿真基准上，CORAL显著超越了传统的联合训练方法，并能有效处理指令歧义。
conclusion: CORAL证明了通过轻量化专家隔离可以有效克服多任务干扰和遗忘问题，是实现大规模持续机器人学习的实用且可扩展的系统。
---
