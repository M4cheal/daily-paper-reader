...
motivation: 现有的世界模型在手工模拟器的可靠性与神经模型的灵活性之间存在断层，难以同时满足易适配、可验证和长时程一致性的需求。
method: 采用DEVS形式化方法，通过分阶段的LLM生成管线，将系统组件的交互结构推理与具体的事件触发及定时逻辑分离。
result: 该方法生成的模型能够产生可验证的结构化事件追踪，并能根据规范派生的时序和语义约束进行自动化诊断与验证。
conclusion: 该研究为离散事件驱动的环境提供了一种兼具一致性、可验证性和高效合成能力的世界模型生成与评估新范式。
---
