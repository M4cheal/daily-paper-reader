...
tags: ["query:tta"]
score: 10.0
evidence: 用于测试时间自适应的前向零阶提示优化
tldr: 针对测试时自适应（TTA）中反向传播方法计算开销大且修改权重的局限，本文提出FOZO。这是一种仅前向传播的零阶提示优化范式，通过优化中间特征统计量和预测熵，并引入动态衰减扰动尺度，实现了在资源受限设备上的高效稳定自适应，在多个数据集上超越了现有梯度及非梯度方法。
selection_source: fresh_fetch
motivation: 现有的TTA方法要么因依赖反向传播而对硬件要求过高，要么因无需反向传播而导致自适应能力受限。
method: 提出一种基于零阶梯度的提示优化方法，结合中间特征统计与预测熵目标，并采用动态衰减的扰动尺度以确保收敛。
result: 在ImageNet-C等多个基准测试中，FOZO的准确率优于主流梯度方法及最先进的仅前向方法，且在量化模型上表现稳健。
