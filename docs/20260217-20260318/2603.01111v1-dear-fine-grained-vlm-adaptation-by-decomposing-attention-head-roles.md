...
tags: ["query:tta"]
score: 8.0
evidence: 通过分解注意力角色进行细粒度VLM自适应
tldr: 针对预训练视觉语言模型在下游任务适配中存在的任务特定知识与零样本泛化能力之间的权衡问题，本文提出了DeAR框架。该方法打破了传统的层级适配观念，通过“概念熵”指标将深层注意力头细分为属性、泛化和混合三种角色。通过角色感知注意力掩码和属性标记，DeAR实现了对信息流的精确控制，在提升特定任务性能的同时，有效保留了模型的通用泛化能力。
selection_source: fresh_fetch
motivation: 现有的层级适配方法导致可学习标记与原始标记间交互失控，难以兼顾任务特定知识的学习与零样本泛化能力的保持。
method: 提出DeAR框架，利用概念熵对注意力头进行角色分解，并结合角色感知注意力掩码和属性标记来精确引导信息流。
result: 在15个数据集上的实验表明，DeAR在任务适配和泛化性能之间取得了最佳平衡，优于现有的主流方法。
conclusion: 通过在更细粒度的注意力头层面进行角色分解和干预，可以更有效地实现视觉语言模型的精准适配与知识保留。
---

## 摘要
提示学习是将预训练视觉语言模型（VLMs）适配到下游任务的主流范式。然而，现有方法通常依赖于一种简单的、以层为中心的观点，即假设浅层捕获通用特征，而深层处理任务特定知识。这种假设导致了可学习标记与原始标记之间不受控制的交互。任务特定知识可能会损害模型的核心泛化能力，并在任务适配与保留零样本泛化能力之间产生权衡。为了解决这一问题，我们挑战了以层为中心的观点，并提出了 DeAR 框架，通过分解注意力头角色（Decomposing Attention head Roles）来实现细粒度的 VLM 适配。我们认为，VLM 内部的功能专业化并非发生在层间，而是在深层中单个注意力头的更细粒度水平上。基于这一见解，我们引入了一种新的度量指标——概念熵（Concept Entropy），用于系统地将注意力头分类为不同的功能角色：属性、泛化和混合。在这些角色的指导下，我们引入了专门的属性标记和基于角色的注意力掩码机制，以精确控制信息流，确保泛化头与任务特定知识保持隔离。我们进一步在推理中加入了一种任务自适应融合策略。在 15 个数据集上的广泛实验表明，DeAR 在任务适配和泛化之间实现了良好的平衡，在各种任务上均优于以往的方法。
