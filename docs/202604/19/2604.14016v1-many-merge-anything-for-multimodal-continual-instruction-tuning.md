...
title_zh: MAny：用于多模态持续指令微调的万物合并框架
...
tldr: 针对多模态大模型在持续指令微调中面临的灾难性遗忘问题，本文揭示了跨模态投影空间的感知漂移和低秩参数空间的推理崩溃这一“双重遗忘”现象。为此，提出了MAny框架，通过跨模态投影合并（CPM）和低秩参数合并（LPM）来融合任务特定知识。该方法无需额外训练，通过高效的代数运算实现最优融合，在多个基准测试中显著提升了模型的持续学习能力。
source: arxiv
...
motivation: 现有研究忽略了多模态模型在持续学习中跨模态投影空间的感知漂移和低秩参数空间的推理崩溃问题。
...
conclusion: MAny作为一种无需训练的参数合并范式，为解决多模态持续指令微调中的知识干扰和遗忘问题提供了高效且数学上最优的方案。
---

## 摘要
多模态持续指令微调（MCIT）对于多模态大语言模型（MLLM）的序列任务自适应至关重要，但受到灾难性遗忘的严重限制。现有文献主要关注推理语言主干，而本研究揭示了一个关键但被忽视的双重遗忘现象，即跨模态投影空间中的感知漂移和低秩参数空间中的推理崩溃。为解决这一问题，我们提出了 MAny（Merge Anything）框架，通过跨模态投影合并（CPM）和低秩参数合并（LPM）来融合特定任务的知识。具体而言，CPM 通过视觉原型引导自适应地合并跨模态视觉表示，从而恢复感知对齐，确保推理过程中的准确特征恢复。同时，LPM 通过递归合并低秩权重矩阵，消除了特定任务低秩模块之间的相互干扰。利用递归最小二乘法，LPM 提供了一个闭式解，从数学上保证了推理稳定性的最优融合轨迹。值得注意的是，MAny 是一种无需训练的范式，通过高效的基于 CPU 的代数运算实现知识合并，消除了初始微调之外的额外基于梯度的优化。我们的广泛评估证实了 MAny 在多个 MLLM 和基准测试中的卓越性能和鲁棒性。具体而言，在 UCIT 基准测试中，MAny 在两种不同的 MLLM 上，最终平均准确率分别比现有最先进方法高出 8.57% 和 2.85%。
