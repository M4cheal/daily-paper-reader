...
tldr: 针对视觉-语言-动作（VLA）模型在长程操作任务中表现不佳的问题，本文提出了HELM框架。该框架通过情节记忆模块检索关键历史，利用状态验证器预测动作失败，并结合控制模块进行回滚与重规划，有效解决了记忆、验证和恢复三大缺口。实验表明，HELM在LIBERO-LONG等基准测试中显著提升了成功率，并增强了模型在扰动下的恢复能力。
source: arxiv
