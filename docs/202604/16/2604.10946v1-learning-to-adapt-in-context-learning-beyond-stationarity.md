...
tldr: 本研究探讨了 Transformer 在非平稳任务分布下的上下文学习（ICL）能力。针对现有理论多假设任务平稳而忽视现实中目标函数随时间变化的问题，本文通过一阶自回归过程建模非平稳回归，重点分析了门控线性注意力（GLA）机制。研究证明 GLA 能通过学习“近期偏置”自适应调节历史信息权重，在动态环境下优于标准线性注意力，为理解 Transformer 处理演化任务的机制提供了理论与实证支持。
source: arxiv
...
motivation: 现有的上下文学习理论大多假设任务分布是平稳的，无法解释模型如何应对现实中随时间变化的目标函数。
method: 采用一阶自回归过程建模非平稳性，并对门控线性注意力（GLA）在动态回归问题中的适应机制进行严谨的理论刻画。
result: 理论与实验均表明，GLA 通过自适应调制过去输入的影响（即学习到的近期偏置），在非平稳任务中实现了比标准线性注意力更低的误差。
conclusion: 门控机制是 Transformer 应对非平稳环境的关键，其实现的自适应学习能力显著提升了模型在动态任务中的表现。
---

## 摘要
由于其强大的经验性能，Transformer 模型已成为广泛科学和工程领域的基础。其成功的关键能力是上下文学习（In-Context Learning, ICL）：当给定来自未知任务的简短提示时，Transformer 可以在不进行任何参数更新的情况下执行逐 token 和下一 token 预测。最近的理论研究已开始揭示这一现象背后的机制，特别是在监督回归设置中。然而，这些分析主要假设任务分布是平稳的，这忽略了目标函数随时间变化的广泛现实场景。在这项工作中，我们通过提供非平稳回归问题下 ICL 的理论分析来填补这一空白。我们研究了门控线性注意力（Gated Linear Attention, GLA）机制如何适应不断变化的输入输出关系，并严格刻画了其在这种动态设置中相对于标准线性注意力的优势。为了对非平稳性建模，我们采用了一阶自回归过程，并证明 GLA 通过自适应地调节过去输入的影响——有效地实现了一种可学习的近期偏差（Recency Bias）——从而实现了更低的训练和测试误差。我们的理论发现得到了实验结果的进一步支持，验证了门控机制在非平稳 ICL 任务中的优势。
