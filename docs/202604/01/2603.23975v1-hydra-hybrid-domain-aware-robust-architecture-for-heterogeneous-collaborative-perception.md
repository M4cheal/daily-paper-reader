...
## 摘要
在协同感知中，由于模型架构或训练数据分布差异所导致的异构性，可能会降低智能体的性能。为了应对这一挑战，我们提出了 HyDRA（混合领域感知鲁棒架构），这是一个在领域感知框架内集成中间融合和后期融合的统一流水线。我们引入了一个轻量级领域分类器，用于动态识别异构智能体并将其分配到后期融合分支。此外，我们提出了锚点引导的位姿图优化，以减轻后期融合中固有的定位误差，利用来自中间融合的可靠检测结果作为固定的空间锚点。大量实验表明，尽管不需要额外的训练，HyDRA 的性能仍可与最先进的异构感知协同感知（CP）方法相媲美。重要的是，随着协作智能体数量的增加，这种性能得以保持，从而实现了无需重新训练的零成本扩展。
