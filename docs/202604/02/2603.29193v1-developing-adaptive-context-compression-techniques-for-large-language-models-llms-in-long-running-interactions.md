...
## 摘要
大语言模型（LLMs）在长时交互过程中，由于上下文长度增加、内存饱和以及计算开销，往往会出现性能下降。本文提出了一种自适应上下文压缩框架，该框架集成了重要性感知记忆选择、连贯性敏感过滤和动态预算分配，旨在保留关键对话信息的同时控制上下文增长。该方法在 LOCOMO、LOCCO 和 LongBench 基准测试上进行了评估，以衡量回答质量、检索准确性、连贯性保持和效率。实验结果表明，与现有的基于记忆和压缩的方法相比，该方法在减少 Token 使用量和推理延迟的同时，在对话稳定性和检索性能方面取得了持续的提升。这些发现表明，自适应上下文压缩在持久性 LLM 交互中，为长期记忆保留与计算效率之间提供了有效的平衡。
