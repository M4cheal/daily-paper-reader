...
tags: ["query:tta"]
score: 6.0
evidence: 智能体通过经验学习对上下文进行迭代更新
tldr: 本文提出Reflective Context Learning (RCL)框架，旨在系统化研究上下文空间中的优化问题。针对智能体在交互学习中面临的信用分配、过拟合等挑战，RCL通过“反射”生成类似梯度的更新信号，并利用“变异”迭代优化上下文。研究将多种上下文优化方法统一为经典优化原语（如批处理、辅助损失等），在AppWorld等基准测试中显著提升了性能，证明了上下文学习应被视为可系统优化的过程。
selection_source: fresh_fetch
motivation: 现有的上下文学习方法缺乏系统性，未能有效解决上下文空间中类似于传统机器学习的信用分配、过拟合和高方差等核心优化难题。
method: 提出RCL框架，通过反射机制将轨迹转化为方向性更新信号，并引入批处理、失败重放、辅助损失等经典优化原语来迭代改进上下文。
result: 在AppWorld、BrowseComp+和RewardBench2等任务上，RCL通过优化原语显著超越了强基准，并验证了不同组件对性能提升的贡献。
conclusion: 上下文学习应被视为一个可以通过借鉴经典优化原则进行系统性研究和改进的优化问题，而非一系列孤立的算法。
---
