...
## 摘要
将预训练语言模型（LMs）适配为视觉语言模型（VLMs）时，由于多模态适配过程中引入的表示偏移和跨模态干扰，可能会降低其原有的语言能力。即使使用标准目标进行针对性的特定任务微调，这种损失也很难恢复。先前的恢复方法通常引入额外的模块作为中间对齐层，以维持或隔离特定模态的子空间，这增加了架构复杂度，增加了推理时的参数量，并限制了在不同模型和设置下的灵活性。我们提出了 LinguDistill，这是一种无适配器（adapter-free）的蒸馏方法，通过利用原始冻结的语言模型作为教师模型来恢复语言能力。我们通过引入逐层 KV 缓存共享（layer-wise KV-cache sharing），克服了实现视觉条件教师监督的关键挑战，该方法在不修改任何模型架构的情况下，使教师模型能够接触到学生模型的多模态表示。然后，我们在语言密集型数据上选择性地蒸馏教师模型强大的语言信号以恢复语言能力，同时保留学生模型在多模态任务上的视觉接地（visual grounding）能力。结果表明，LinguDistill 恢复了在语言和知识基准测试中损失的约 10% 的性能，同时在重视觉任务上保持了相当的性能。我们的研究结果表明，无需额外模块即可恢复语言能力，为多模态模型中特定模态的退化问题提供了一种高效且实用的解决方案。
