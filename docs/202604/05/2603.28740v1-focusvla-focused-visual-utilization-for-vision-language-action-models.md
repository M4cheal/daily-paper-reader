...
tags: ["query:tta"]
score: 6.0
evidence: 提升视觉语言动作（VLA）模型中的视觉利用率
tldr: 针对视觉-语言-动作（VLA）模型在处理视觉细节时存在的架构偏差、冗余Token及噪声干扰等瓶颈，本文提出FocusVLA。该模型通过模态级联注意力机制消除捷径路径，强制模型关注视觉细节，并利用聚焦注意力机制动态筛选任务相关的视觉块以抑制噪声。实验证明，FocusVLA在模拟和真实机器人任务中显著提升了动作生成的准确性与收敛速度。
selection_source: fresh_fetch
motivation: 现有VLA模型因架构偏差、视觉Token过载和任务无关噪声，导致模型难以有效利用关键视觉细节来指导动作生成。
method: 提出FocusVLA架构，包含强制模型依赖视觉细节的模态级联注意力，以及动态筛选关键视觉区域并抑制噪声的聚焦注意力。
result: 在模拟和真实世界机器人基准测试中，FocusVLA显著增强了灵巧操作能力，并大幅提升了任务成功率与训练收敛速度。
conclusion: 研究表明VLA性能瓶颈在于视觉信息的利用方式而非表征质量，通过聚焦关键视觉区域可有效提升动作生成质量。
---

## 摘要
视觉-语言-动作（VLA）模型通过将策略以丰富的视觉-语言信息为条件来改进动作生成。然而，当前的自回归策略受到三个瓶颈的制约：(1) 架构偏差导致模型忽视视觉细节；(2) 过多的视觉 token 使得注意力难以聚焦在正确区域；(3) 与任务无关的视觉信息引入了大量噪声——这些因素共同严重损害了动作质量。在本文中，我们研究了如何有效地利用不同的视觉表示进行动作生成。为此，我们首先通过实验验证了上述问题，并表明 VLA 的性能主要受限于视觉信息的利用方式，而非视觉表示的质量。基于这些见解，我们提出了 FocusVLA，这是一种将模型注意力引导至任务相关视觉区域的新型范式，从而有效地桥接视觉与动作。具体而言，我们首先提出了模态级联注意力（Modality Cascaded Attention）以消除捷径路径，从而迫使 VLA 模型依赖任务相关的视觉细节进行动作生成。此外，我们提出了聚焦注意力（Focus Attention），它动态选择任务相关的视觉补丁（patches）以控制信息量，同时显式调节其影响力以抑制任务无关的噪声。在模拟和真实世界机器人基准测试上的广泛实验表明，FocusVLA 不仅能有效利用视觉细节执行灵巧操作，还能在各种任务中显著提升性能并加速收敛。
