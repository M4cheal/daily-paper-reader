...
motivation: 针对现有VLA模型因庞大的骨干网络和复杂的动作生成头导致的推理延迟高、计算成本昂贵且难以实时部署的问题。
method: 引入预算感知的自适应推理方案，通过监测中间层动作一致性实现骨干网络早期停止，并结合跨层截断流匹配技术加速去噪过程。
result: "在RoboChallenge等多个基准测试中超越了pi0和RDT-1B等基准模型，同时最高减少了72%的推理延迟和76.6%的骨干网络计算量。"
conclusion: A1证明了通过自适应截断和高效流匹配技术，可以在保证机器人操作成功率的同时大幅提升VLA模型的推理效率和部署可行性。
---
