...
## 摘要
在运行设计域（ODD）之外的情况下确保可靠性能，仍然是设计韧性自主系统的主要挑战。我们通过引入一种自适应概率系统模型的方法来应对这一挑战，以处理超出 ODD 的场景，同时提供定量保证。我们的方法动态地扩展了现有系统情境能力的覆盖范围，支持在非预期情况下对系统行为进行验证和自适应。初步结果表明，即使在不可预见的超出 ODD 的情况下，我们的方法也能通过调整系统行为并提供形式化保证，有效地提高系统的可靠性。
