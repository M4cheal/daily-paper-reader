...
tldr: 针对视觉上下文学习（Visual ICL）模型缺乏交互性的局限，本文提出了Interactive DeLVM。该方法通过将用户提供的涂鸦、点击或框选等交互信号直接编码进示例输入输出对中，使静态模型转变为可交互系统。无需微调即可实现用户驱动的任务引导，在交互式分割、超分辨率和物体移除等任务上显著优于现有SOTA模型，填补了静态任务自适应与流体交互之间的空白。
selection_source: fresh_fetch
motivation: 现有的视觉上下文学习模型虽然能适应新任务，但无法响应用户提供的实时引导信号（如点击或涂鸦），限制了其在实际应用中的灵活性。
method: 提出Interactive DeLVM，通过将交互线索直接编码到示例对中，使模型能够在不改变核心架构的情况下，利用视觉提示动态引导预测过程。
result: "在交互式分割、定向超分辨率和物体移除任务中，该方法分别提升了7.95% IoU、2.46 PSNR并降低了3.14% LPIPS，显著增强了模型的可控性。"
conclusion: 本研究成功将静态视觉上下文学习扩展为用户驱动的交互式系统，实现了在无需微调的情况下通过自然视觉提示精确控制模型输出。
---

## 摘要
视觉上下文学习模型旨在通过利用一组示例输入-输出对来适应新任务，从而在无需针对特定任务进行微调的情况下实现快速泛化。然而，这些模型在根本上以静态范式运行：虽然它们可以适应新任务，但缺乏任何机制来整合用户提供的引导信号（如涂鸦、点击或边界框），以引导或细化预测过程。这种局限性在实际应用中尤为突出，因为用户希望主动引导模型预测，例如通过突出显示目标对象进行分割、指示应进行视觉修改的区域，或在复杂场景中隔离特定人物以进行针对性的姿态估计。在这项工作中，我们提出了一种简单的方法，将静态视觉上下文学习器（特别是 DeLVM 方法）转变为高度可控的用户驱动系统，即交互式 DeLVM，从而通过涂鸦、点击或绘制框等自然视觉提示实现无缝交互。具体而言，通过将交互直接编码到示例输入-输出对中，我们保持了视觉上下文学习的理念：使用户能够在无需微调的情况下使用未见过的交互来提示模型，并赋予他们通过个性化交互动态引导模型预测的能力。我们的实验表明，最先进的（SOTA）视觉上下文学习模型无法有效利用交互提示，通常会完全忽略用户引导。相比之下，我们的方法在可控的用户引导场景中表现出色，在交互式分割中实现了 +7.95% 的 IoU 提升，在定向超分辨率中实现了 +2.46 的 PSNR 提升，在交互式物体移除中实现了 -3.14% 的 LPIPS 降低。通过这项工作，我们弥合了僵化的静态任务适应与以用户为中心的视觉上下文学习的流畅交互性之间的鸿沟。
