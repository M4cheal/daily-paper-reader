...
title_zh: 学习搜索：一种用于基于知识的视觉问答的基于决策的智能体
...
## 摘要
基于知识的视觉问答（KB-VQA）要求视觉语言模型理解图像并利用外部知识，特别是针对稀有实体和长尾事实。大多数现有的检索增强生成（RAG）方法采用固定的流水线，按顺序检索信息、过滤信息，然后生成答案。这种设计使其难以适应多样化的提问类型。此外，它将检索与推理分离，使得模型难以决定何时搜索、如何优化查询或何时停止。结果，检索到的证据往往与问题对齐不佳。为了解决这些局限性，我们将 KB-VQA 重新表述为一个搜索智能体问题，并将解决过程建模为一个多步决策程序。在每一步中，智能体根据其当前的信息状态，从四个动作（回答、图像检索、文本检索和生成描述）中选择其一。我们进一步设计了一个自动化流水线来收集多步轨迹，记录智能体的推理过程、工具使用和中间决策。这些轨迹随后被用作微调的监督信号。在 InfoSeek 和 E-VQA 上的实验表明，我们的方法达到了最先进的性能，持续优于之前的基准模型，并证实了我们框架的有效性。
