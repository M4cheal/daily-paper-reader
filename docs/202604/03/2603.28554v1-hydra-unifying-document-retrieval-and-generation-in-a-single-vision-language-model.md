...
tags: ["query:tta"]
score: 6.0
evidence: 在推理时切换适配器以应对VQA基准测试
tldr: "针对视觉文档理解中检索与生成模型分离导致的内存和系统复杂性问题，本文提出 Hydra 框架。通过在单个视觉语言模型（VLM）中引入可切换的 LoRA 适配器，Hydra 实现了 ColBERT 式的多向量检索与自回归生成的统一。该方法在保持生成质量几乎无损的同时，将 GPU 峰值内存降低了 41%，并成功扩展至音频和视频等多模态检索任务。"
selection_source: fresh_fetch
motivation: 传统的视觉文档理解需要独立的检索和生成模型，这显著增加了系统的内存开销和维护复杂度。
method: 采用双头架构，通过一个仅针对检索训练的 LoRA 适配器，在推理时动态切换以实现多向量嵌入生成或原始模型的文本生成。
result: "Hydra 在 ViDoRe 基准测试上表现优异，且在保持生成输出与基座模型高度一致的同时，减少了 41% 的 GPU 内存占用。"
conclusion: Hydra 证明了通过简单的适配器切换机制即可在单一 VLM 中高效统一检索与生成功能，且具备良好的多模态扩展潜力。
---
