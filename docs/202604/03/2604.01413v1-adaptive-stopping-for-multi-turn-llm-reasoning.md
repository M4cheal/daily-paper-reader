...
tags: ["query:tta"]
score: 6.0
evidence: 大语言模型多轮推理中的自适应停止
tldr: 针对多轮推理（如RAG和ReAct）中缺乏可靠停止准则的问题，本文提出了MiCP框架。这是首个将符合预测（CP）应用于多轮推理的框架，通过在不同轮次分配错误预算，实现了在保证预测覆盖率的同时允许模型提前停止。实验证明MiCP在多个问答基准上能有效降低推理成本和轮次，并为高风险领域的应用提供了正式的可靠性保证。
selection_source: fresh_fetch
motivation: 现有多轮推理方法依赖启发式停止规则，缺乏正式的准确性保证，导致在处理复杂任务时难以平衡推理成本与预测风险。
method: 提出MiCP框架，通过在多轮推理过程中动态分配错误预算，利用符合预测技术为自适应停止提供形式化的覆盖率保证。
result: 在RAG和ReAct等任务中，MiCP在满足预设覆盖率目标的同时，显著减少了推理轮次、计算成本和预测候选集的大小。
conclusion: MiCP为多轮LLM推理提供了一种兼顾效率与可靠性的通用框架，并引入了联合评估覆盖率与效率的新度量指标。
---
