...
tags: ["query:tta"]
score: 9.0
evidence: 对多模态视觉语言模型CLIP的测试时自适应
tldr: 该论文针对视觉语言模型CLIP在领域偏移下的泛化问题，提出了基于权重平均的测试时自适应方法WATT。该方法通过多样模板生成伪标签进行模型更新，并利用权重平均整合全局信息，同时引入文本集成策略提升性能。实验结果表明WATT有效提升了CLIP在零样本分类中的鲁棒性，为多模态模型的测试时自适应提供了新思路。
...
motivation: CLIP等视觉语言模型在领域偏移下泛化能力不足，需要测试时自适应来提升鲁棒性。
method: 采用多样文本模板生成伪标签，利用预测作为伪标签进行模型更新，并通过权重平均和文本集成策略聚合信息。
result: WATT在多种领域偏移场景下显著提升了CLIP的分类准确率，验证了方法的有效性。
conclusion: 权重平均和文本集成能有效提升多模态模型测试时自适应的性能，为后续研究提供了新方向。
---
