...
title_zh: ReservoirTTA：面向演化和循环领域的长时间测试时自适应
...
tags: ["query:tta"]
score: 9.0
evidence: 面向流数据的长期在线测试时自适应
tldr: 针对长时间测试时适应中单模型容易遗忘和干扰的问题，本文提出ReservoirTTA，维护一个领域专家模型池。通过在线聚类检测新领域并路由样本到相应模型，实现领域专用适应。该方法有效克服了灾难性遗忘和错误累积，在演化和循环领域场景下性能稳定。
...
motivation: 单模型在长时间域偏移中会灾难性遗忘和错误累积。
method: 提出模型池和在线聚类路由机制，实现领域专用适应。
result: 在多个演化和循环域基准上优于单模型方法。
conclusion: ReservoirTTA为长时域适应提供了可扩展的即插即用解决方案。
---
