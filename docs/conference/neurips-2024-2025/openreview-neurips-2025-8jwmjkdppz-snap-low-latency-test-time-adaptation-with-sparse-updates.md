...
title_zh: SNAP：低延迟稀疏测试时自适应
authors: "Hyeongheon Cha, Dong Min Kim, Hye Won Chung, Taesik Gong, Sung-Ju Lee"
date: 2025-09-18
pdf: "https://openreview.net/pdf?id=8JwMjKDppz"
tags: ["query:tta"]
score: 9.0
evidence: 面向流数据的稀疏测试时自适应，低延迟
tldr: "流数据场景下频繁自适应计算成本高。SNAP通过稀疏更新策略，仅用1%数据即可保持高精度，提出类域代表性内存识别关键样本。在边缘设备上实现低延迟，为在线测试时自适应提供高效方案。"
...
motivation: 现有方法依赖频繁更新，资源消耗大，不适合边缘环境。
method: 提出稀疏更新框架，含类域代表性内存和自适应调度。
result: "适应频率降低99%仍保持高精度，适用于资源受限设备。"
conclusion: 稀疏更新可显著降低测试时自适应计算成本，推动实际部署。
---
