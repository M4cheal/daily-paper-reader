...
title_zh: 在线机器学习和测试时间自适应的参数动态
authors: Jae-Hong Lee
date: 2025-09-18
pdf: "https://openreview.net/pdf?id=pdlepT2alu"
tags: ["query:tta"]
score: 9.0
evidence: 在线测试时间自适应与参数动态分析
tldr: 在线机器学习中非独立同分布环境导致模型参数不稳定，现有TTA方法忽略此问题。本文提出基于随机微分方程的概率框架，建模自适应过程中的参数动态。该框架提供对非独立同分布下参数演化的理论分析。实验验证了框架对在线TTA稳定性的解释与改进能力。
...
motivation: 在线学习中独立同分布假设被破坏导致TTA不稳定，现有方法忽视参数动态风险。
method: 使用随机微分方程对自适应过程建模，实现参数动态的理论分析。
result: 框架揭示了非独立同分布下参数变化规律，并指导了更稳定的在线TTA算法。
conclusion: 为在线TTA提供了新的理论工具，有助于设计更鲁棒的适应策略。
---
