...
title_zh: 面向开放词汇语义分割的视觉语言模型测试时自适应
...
tldr: 测试时自适应在视觉语言模型的密集预测任务中尚未被充分研究。本文提出MLMP方法，通过多层次和多提示熵最小化，在测试时适应视觉语言模型进行开放词汇语义分割，可作为即插即用模块提升分割性能。
...
motivation: 填补测试时自适应在开放词汇语义分割中的空白。
method: 提出多层次多提示熵最小化方法，结合中间层特征与文本提示模板进行适应。
result: 在多个分割数据集上提升了开放词汇语义分割的准确性。
conclusion: 为视觉语言模型在密集预测任务中的测试时自适应提供了新方案。
---
