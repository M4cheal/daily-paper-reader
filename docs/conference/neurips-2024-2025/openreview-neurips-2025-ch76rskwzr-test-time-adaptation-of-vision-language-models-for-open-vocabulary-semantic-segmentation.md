...
title_zh: 视觉语言模型的测试时间自适应用于开放词汇语义分割
...
tags: ["query:tta"]
score: 9.0
evidence: 提出面向开放词汇语义分割的视觉语言模型测试时间自适应方法
tldr: 测试时间自适应在密集预测任务中研究不足。本文针对开放词汇语义分割提出MLMP方法，在测试时通过多层级视觉特征和多提示模板的熵最小化自适应调整模型，同时利用全局CLS令牌和局部像素级信息。该方法即插即用，显著提升分割性能，为测试时间自适应在指代分割等任务中的应用奠定了直接基础。
...
motivation: 测试时间自适应在密集预测任务如开放词汇语义分割中尚未被充分探索。
method: 提出多层级多提示熵最小化，整合中间视觉特征和多种文本提示模板进行测试时自适应。
result: 在多个开放词汇分割基准上实现即插即用的性能提升，优于现有TTA方法。
conclusion: 该工作填补了TTA在密集预测中的空白，可直接扩展至指代分割等任务。
---
