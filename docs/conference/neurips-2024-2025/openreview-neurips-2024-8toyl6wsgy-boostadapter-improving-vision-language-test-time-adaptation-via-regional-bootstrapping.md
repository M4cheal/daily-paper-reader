...
score: 9.0
evidence: 视觉语言测试时自适应
tldr: 针对有训练和无训练TTA方法的各自局限，本文提出BoostAdapter，通过区域引导机制融合两者优势。该方法维护轻量级自适应模块，利用测试样本的区域信息进行引导，避免了完整反向传播。实验表明，BoostAdapter在多个下游任务上取得优异性能，且计算开销低。
...
motivation: 现有TTA方法在有训练和无训练范式间存在鸿沟，各有弊端。
method: 提出区域引导框架，结合轻量级自适应模块和测试样本信息。
result: 在多个视觉语言任务上达到SOTA，且效率高。
conclusion: BoostAdapter有效弥合了有训练和无训练TTA的差距。
---
