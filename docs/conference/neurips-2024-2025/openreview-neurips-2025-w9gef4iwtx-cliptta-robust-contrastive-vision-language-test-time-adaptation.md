...
tags: ["query:tta"]
score: 9.0
evidence: 面向视觉语言模型的对比测试时自适应及理论分析
tldr: 现有测试时自适应采用熵最小化，与视觉语言模型对比训练目标不匹配。CLIPTTA提出软对比损失，与CLIP预训练对齐，理论分析证明其可避免伪标签漂移和类别坍缩。实验在多个分布偏移基准上提升性能，提供新的TTA范式。
...
motivation: 熵最小化与视觉语言模型的对比训练目标不一致。
method: 提出基于软对比损失的梯度测试时自适应方法。
result: 在多个视觉语言分布偏移任务上优于现有方法，避免坍缩。
conclusion: 对比测试时自适应更符合视觉语言模型本质，提升泛化性。
---
