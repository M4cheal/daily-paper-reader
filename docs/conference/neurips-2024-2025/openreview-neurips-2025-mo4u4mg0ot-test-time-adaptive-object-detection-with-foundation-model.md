...
tags: ["query:tta"]
score: 8.0
evidence: 多模态模型的测试时自适应
tldr: 针对现有测试时自适应目标检测依赖源数据和封闭类别假设的问题，本文首次引入基础模型，提出多模态提示均值教师框架，无需源数据且支持开放类别。方法结合文本和视觉提示，利用视觉语言检测器进行在线适应。实验在多个域偏移场景下取得了优异性能。
...
motivation: 现有方法依赖源统计特征且假设类别空间相同，限制实际应用。
method: 基于多模态提示的均值教师框架，结合基础模型实现零源数据适应。
result: 在多个域适应目标检测基准上超过现有方法，且支持开放类别。
conclusion: 该工作为无需源数据的开放世界测试时自适应检测奠定了基础。
---
