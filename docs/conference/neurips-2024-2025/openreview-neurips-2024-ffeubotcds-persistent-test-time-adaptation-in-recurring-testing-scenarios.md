...
title_zh: 重复测试场景下的持久测试时自适应
authors: "Trung-Hieu Hoang, MinhDuc Vo, Minh N. Do"
date: 2024-09-25
pdf: "https://openreview.net/pdf?id=ffeUBoTcdS"
tags: ["query:tta"]
score: 8.0
evidence: 重复环境中TTA的理论分析（高斯混合模型）
tldr: 现有TTA方法能否在长时间内保持自适应能力尚不清楚。本文提出重复TTA诊断设定，模拟环境变化且重复出现的数据流。在高斯混合模型分类器上从理论上分析了数据集和算法相关的误差累积因素，揭示了TTA在长时间尺度下的行为。
...
motivation: 现有TTA研究缺乏对长期自适应能力的理论理解，尤其是在环境重复出现的场景下。
method: 提出重复TTA诊断设定，并在扰动高斯混合模型分类器上进行理论推导，分析误差累积。
result: 理论揭示了TTA模型在重复环境中的误差累积特性，为设计更鲁棒的TTA方法提供指导。
conclusion: 本研究为TTA的长期稳定性提供了理论基础，强调了重复环境的影响。
---
