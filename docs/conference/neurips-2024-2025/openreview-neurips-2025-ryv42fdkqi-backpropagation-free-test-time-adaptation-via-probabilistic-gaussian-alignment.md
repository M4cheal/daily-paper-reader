...
title_zh: 基于概率高斯对齐的免反向传播测试时间自适应
...
tags: ["query:tta"]
score: 8.0
evidence: 免反向传播的测试时间自适应
tldr: 现有TTA方法依赖反向传播或迭代优化，限制了实时部署和可扩展性。本文提出ADAPT，一种免反向传播的TTA方法，将问题重构为概率高斯对齐。它显式建模类条件特征分布，无需源数据和标签。理论分析保证其有效性，实验表明在多个分布偏移任务上达到甚至超过现有方法，且速度显著提升。
...
motivation: 现有TTA依赖反向传播，计算开销大且未显式建模类条件分布。
method: 将TTA重新定义为概率高斯对齐，无需梯度计算。
result: 在图像分类等任务上取得与反向传播方法相当或更优的结果，速度提高数倍。
conclusion: 提供了一种高效且理论上可靠的TTA替代方案。
---
