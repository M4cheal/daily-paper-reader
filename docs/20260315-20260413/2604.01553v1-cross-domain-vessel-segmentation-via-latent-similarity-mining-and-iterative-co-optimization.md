...
tags: ["query:tta-seg"]
score: 9.0
evidence: 通过潜在相似性挖掘处理领域偏移的跨域视网膜血管分割
tldr: 针对视网膜血管分割在跨领域应用中因领域偏移导致的性能下降问题，本文提出了一种结合潜在相似性挖掘与迭代协同优化的领域迁移框架。该方法利用条件扩散模型建立跨领域的潜在表示，并通过生成网络与分割网络的循环互优化，显著提升了跨领域图像合成质量和分割精度，在多种临床场景下达到了SOTA水平。
source: arxiv
...
method: 提出一种利用条件扩散模型提取跨领域潜在相似性，并通过生成与分割网络的迭代协同优化来增强模型泛化能力的框架。
result: 实验证明该框架在跨领域视网膜血管分割任务中取得了当前最先进的性能，尤其在模态差异显著的临床场景中表现优异。
conclusion: 通过挖掘潜在血管相似性并实施生成与分割任务的协同进化，可以有效解决医学影像分割中的领域自适应难题。
---

## 摘要
视网膜血管分割是视网膜病变自动诊断的关键前提。尽管卷积神经网络（CNN）的最新进展在该任务中表现出良好的性能，但当训练数据与测试数据之间存在域偏移时，性能会出现显著下降。为解决这些局限性，我们提出了一种新型域迁移框架，该框架利用了跨域的潜在血管相似性，并对生成网络和分割网络进行迭代协同优化。具体而言，我们首先为源域和目标域预训练生成网络。随后，预训练的源域条件扩散模型执行确定性反演，以建立血管图像的中间潜在表示，从而为目标合成创建域无关的原型。最后，我们开发了一种迭代细化策略，通过循环参数更新使分割网络和生成模型进行相互优化。这种协同演化过程能够同时提升跨域图像合成质量和分割精度。实验表明，我们的框架在跨域视网膜血管分割中达到了最先进的性能，特别是在具有显著模态差异的挑战性临床场景中。
