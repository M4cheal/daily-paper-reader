...
## 摘要
常驻关键词检测 (KWS) 要求进行设备端自适应，以在严格的延迟和能耗预算下应对特定于用户和环境的分布偏移。本文首次提出将权重自适应（即设备端训练）与架构自适应（以在线结构化通道剪枝的形式）相结合，用于个性化设备端 KWS。从最先进的自学习个性化 KWS 流水线出发，我们比较了应用于现场伪标签用户数据的数据无关和数据感知剪枝准则。在 HeySnips 和 HeySnapdragon 数据集上，在同等任务性能（以每小时 0.5 次误报时的准确率衡量）下，与未剪枝的基准模型相比，我们实现了高达 9.63 倍的模型尺寸压缩。当在 Jetson Orin Nano 嵌入式 GPU 上部署我们的自适应流水线时，与仅权重自适应相比，我们在在线训练/推理过程中分别实现了高达 1.52x/1.57x 和 1.64x/1.77x 的延迟和能耗改进。
