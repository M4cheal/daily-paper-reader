...
tags: ["query:tta"]
score: 9.0
evidence: 测试时分布偏移下的可靠性
tldr: 针对模型合并在测试时分布偏移下预测偏差和泛化性下降的问题，本文提出BD-Merging框架。该框架通过联合证据头建模不确定性，并利用邻接差异评分（ADS）指导对比学习来优化特征表示。通过训练去偏路由实现样本级的自适应权重分配，在无需原始数据的情况下显著提升了多任务学习在复杂分布下的鲁棒性。
selection_source: fresh_fetch
motivation: 现有模型合并方法通常假设测试数据与训练分布一致，导致在实际应用中面临分布偏移时预测结果存在偏差且可靠性不足。
method: 引入联合证据头建模不确定性，并利用邻接差异评分指导对比学习，从而训练出能根据样本特征动态分配权重的去偏路由。
result: 在多种任务上的实验表明，BD-Merging在处理分布偏移时比现有最先进的模型合并基准具有更高的有效性和鲁棒性。
conclusion: 该研究通过显式建模不确定性与动态路由机制，为构建可靠且具备分布鲁棒性的多任务模型合并提供了有效的解决方案。
---

## 摘要
模型合并（MM）已成为多任务学习（MTL）的一种可扩展范式，能够在不重新访问原始训练数据的情况下集成多个特定任务模型。尽管最近取得了进展，但 MM 在测试时分布偏移下的可靠性仍未得到充分理解。大多数现有的 MM 方法通常假设测试数据是干净的，且在分布上与训练源和辅助源对齐。然而，这一假设在实践中很少成立，往往导致预测偏差和泛化性能下降。为了解决这个问题，我们提出了 BD-Merging，这是一个偏差感知的无监督模型合并框架，通过显式建模不确定性来实现分布偏移下的自适应可靠性。首先，BD-Merging 引入了一个联合证据头（joint evidential head），在统一的标签空间上学习不确定性，从而捕获 MM 中的跨任务语义依赖关系。其次，基于这一证据基础，我们提出了邻接差异评分（ADS），用于量化相邻样本之间的证据对齐程度。第三，在 ADS 的引导下，一种差异感知对比学习机制通过对齐一致样本并分离冲突样本来优化合并后的表示。结合通用的无监督学习，该过程训练了一个去偏差路由（debiased router），能够针对每个样本自适应地分配特定任务或特定层的权重，从而有效减轻分布偏移的负面影响。在不同任务上的广泛实验表明，与最先进的 MM 基准方法相比，BD-Merging 具有更优越的有效性和鲁棒性。
