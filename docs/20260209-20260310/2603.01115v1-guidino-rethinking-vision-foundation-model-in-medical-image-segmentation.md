...
tags: ["query:tta"]
score: 8.0
evidence: 在领域偏移下将基础模型适配于医学图像分割
tldr: GuiDINO 针对视觉基础模型在医学图像分割中的领域偏移问题，提出将其作为视觉引导生成器而非直接微调。通过 TokenBook 机制将 DINOv3 特征转化为空间引导掩码，并利用该掩码调节专用分割网络的特征激活。该方法结合了基础模型的先验知识与医学影像架构的归纳偏置，在多个数据集上显著提升了分割精度和边界鲁棒性，为基础模型服务医疗视觉提供了新视角。
selection_source: fresh_fetch
motivation: 视觉基础模型在医学影像中存在领域偏移，直接微调或轻量化适配难以充分发挥其在特定分割任务中的潜力。
method: 引入 GuiDINO 框架，利用 TokenBook 机制将 DINOv3 特征转换为空间引导掩码，并通过门控机制将其注入到医学专用分割网络中。
result: 在多种医学影像数据集和 nnUNet 风格的推理中，GuiDINO 显著提升了分割质量和边界鲁棒性。
conclusion: 将基础模型重新定位为视觉引导生成器，是提升医学图像分割性能和鲁棒性的一种高效且实用的替代方案。
---
