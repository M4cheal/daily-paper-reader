...
tldr: 针对测试时自适应（TTA）中反向传播方法计算开销大且修改模型权重的局限，本文提出FOZO。这是一种仅前向传播的零阶提示优化方法，通过优化中间特征统计量和预测熵，并引入动态衰减扰动尺度来确保稳定性。FOZO在多个ImageNet变体数据集上超越了主流梯度方法和SOTA前向方法，且在量化模型上表现优异，为资源受限场景提供了高效方案。
selection_source: fresh_fetch
motivation: 现有的TTA方法在低端设备上部署困难，因为反向传播计算量大且传统无反向传播方法自适应能力有限。
method: 提出一种仅前向的零阶提示优化范式，通过优化特征统计和预测熵，并结合动态衰减的扰动尺度实现稳定更新。
result: "在ImageNet-C等多个基准测试中，FOZO的准确率达到59.52%，显著优于主流梯度方法及现有的前向自适应方法。"
conclusion: FOZO证明了零阶优化在测试时自适应中的有效性，是资源受限环境下模型部署的一种极具竞争力的解决方案。
---
