...
tldr: 针对多模态大模型在流式视频理解中面临的视觉标记爆炸和内存溢出问题，本文提出 CurveStream 框架。该方法无需训练，通过几何曲率识别特征轨迹中的关键语义转换点，并利用动态阈值实现分级视觉记忆管理。在有限的标记预算下，CurveStream 显著提升了流式视频的感知能力，在多个基准测试中刷新了 SOTA 记录。
selection_source: fresh_fetch
motivation: 现有的流式视频内存管理方法缺乏语义感知，难以捕捉关键的语义转换并维持上下文连贯性。
method: 提出一种基于曲率感知的层次化视觉记忆管理框架，利用曲率得分和动态阈值将帧自适应分配至清晰或模糊记忆状态。
result: "在 StreamingBench 和 OVOBench 等多个基准上，该方法比基线模型性能提升超过 10%，达到了当前领先水平。"
