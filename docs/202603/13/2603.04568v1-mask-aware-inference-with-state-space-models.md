...
## 摘要
许多现实世界的计算机视觉任务（如深度补全）必须处理包含任意形状缺失或无效数据区域的输入。对于卷积神经网络（CNN），部分卷积（Partial Convolutions）通过仅基于有效像素的掩码感知重归一化解决了这一问题。近年来，像 Mamba 这样的状态空间模型（SSM）应运而生，以线性复杂度提供了高性能。然而，这些架构在推理时缺乏处理此类任意形状无效数据的内在机制。为了填补这一空白，我们引入了部分视觉 Mamba（Partial Vision Mamba, PVM），这是一种将部分操作原理移植到 Mamba 骨干网络的新型架构组件。我们还定义了一系列使用 PVM 设计架构的规则。我们在深度补全、图像修复以及带有无效数据的分类任务中展示了该方法的有效性和泛化能力。
