...
tags: ["query:tta"]
score: 8.0
evidence: 测试时梯度下降以实时优化策略
tldr: 针对现有大语言模型推理时计算扩展方法（如离散搜索）效率低下的问题，本文提出 $\nabla$-Reasoner 框架。该框架引入可微文本优化（DTO）技术，在解码过程中利用模型似然和奖励模型的梯度信号，在潜在空间内通过梯度下降迭代优化 Token 表示。该方法实现了从零阶搜索到一阶优化的范式转变，在显著提升推理性能的同时降低了计算开销。
selection_source: fresh_fetch
motivation: 现有的推理时扩展方法多依赖低效的离散搜索或试错提示，难以实现最优的在线策略改进。
method: 提出 $\nabla$-Reasoner 框架，通过可微文本优化（DTO）在解码循环中利用梯度下降实时精炼文本表示。
result: "在数学推理基准测试中准确率提升超过 20%，且相比强基线减少了 10-40% 的模型调用次数。"
conclusion: 该研究证明了在推理阶段采用一阶梯度优化是提升大语言模型推理能力的一种高效且低成本的新路径。
---
