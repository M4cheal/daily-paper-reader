...
motivation: 现有模型在测试时自我进化主要依赖固有推理能力，缺乏针对该任务的显式训练。
method: 提出LSE框架，将上下文编辑简化为单步强化学习目标，并配合树引导的进化循环进行优化。
result: 4B参数模型在BIRD和MMLU-Redux上优于GPT-5、Claude 3.5等驱动的策略及TextGrad等优化方法。
conclusion: 将自我进化作为一种可学习的技能进行训练，能显著提升模型在测试时的上下文优化能力和迁移性。
---
