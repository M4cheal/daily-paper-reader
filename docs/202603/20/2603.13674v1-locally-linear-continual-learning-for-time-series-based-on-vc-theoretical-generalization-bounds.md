...
## 摘要
大多数机器学习方法假设概率分布是固定的，这限制了它们在非平稳现实场景中的适用性。虽然持续学习方法解决了这一问题，但目前的方法通常依赖于黑盒模型，或者为了可解释性而需要大量的人工干预。我们提出了 SyMPLER（通过分段线性演化回归进行系统建模），这是一种基于动态分段线性逼近的非平稳环境下时间序列预测的可解释模型。与其他局部线性模型不同，SyMPLER 利用统计学习理论中的泛化界，根据预测误差自动确定何时添加新的局部模型，从而消除了对数据进行显式聚类的需求。实验表明，SyMPLER 在保持人类可理解结构的同时，能够达到与黑盒模型及现有可解释模型相当的性能，并揭示有关系统行为的见解。从这个意义上说，我们的方法兼顾了准确性和可解释性，为非平稳时间序列预测提供了一种透明且自适应的解决方案。
