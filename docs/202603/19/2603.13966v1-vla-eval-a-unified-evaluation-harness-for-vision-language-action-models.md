...
tldr: 针对视觉-语言-动作（VLA）模型评估中存在的脚本碎片化、依赖冲突和协议不明确等问题，本文推出了开源评估框架 vla-eval。该框架通过 WebSocket 协议和 Docker 隔离技术，实现了模型推理与基准测试环境的解耦。它支持 13 个基准测试，通过并行化技术将评估效率提升了 47 倍，并提供了包含 657 条结果的排行榜，显著提升了 VLA 模型评估的标准化与复现效率。
selection_source: fresh_fetch
motivation: 现有的 VLA 模型评估缺乏统一标准，各模型库独立维护脚本导致代码冗余、依赖冲突且实验协议不透明。
method: 提出一种基于 WebSocket 和 Docker 的解耦架构，通过统一接口实现模型与基准测试的自动交叉评估，并支持并行分片推理。
result: 框架支持 13 个基准测试，评估吞吐量提升 47 倍，并成功复现了已发表模型的性能，同时揭示了影响结果的隐藏因素。
conclusion: vla-eval 为 VLA 领域提供了标准化的评估基础设施和排行榜，有助于推动模型性能的公平比较与科研复现。
---
