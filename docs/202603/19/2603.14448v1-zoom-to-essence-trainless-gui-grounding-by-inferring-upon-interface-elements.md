...
tldr: 针对多模态大模型在GUI定位中过度依赖大规模微调且成本高昂的问题，本文提出了ZoomUI。这是一种无需训练的新方法，通过推理缩放引导通用模型逐步定位目标。它首先将自然语言指令转化为具体的视觉特征描述，随后利用内部注意力机制迭代缩放界面区域以锁定目标元素。实验表明，该方法在多个基准测试中达到或超越了现有最先进水平。
selection_source: fresh_fetch
motivation: 现有的GUI定位方法通常需要在大规模数据集上进行昂贵的微调，且性能高度依赖于训练数据的质量和分布。
method: 提出ZoomUI框架，通过优化潜在思维将指令转化为视觉描述，并结合内部注意力机制实现对目标UI元素的迭代缩放定位。
result: 在广泛的基准测试评估中，ZoomUI在无需额外训练的情况下，性能达到甚至超过了当前最先进的SOTA基线模型。
conclusion: 该研究证明了通过推理阶段的优化和界面元素分解，通用多模态大模型无需微调即可实现高效且精准的GUI定位。
---
