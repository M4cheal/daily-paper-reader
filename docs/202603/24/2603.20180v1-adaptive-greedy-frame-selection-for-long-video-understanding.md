...
## 摘要
大视觉语言模型（VLM）正越来越多地应用于长视频问答，但推理过程往往受限于输入帧数及由此产生的视觉 Token 数量。朴素的稀疏采样可能会遗漏关键时刻，而纯粹由相关性驱动的选择往往会陷入近乎重复的帧，并牺牲对时间跨度较大的证据的覆盖。我们提出了一种问题自适应的贪婪帧选择方法，在固定帧预算下联合优化查询相关性和语义代表性。我们的方法构建了一个 1 FPS 的候选池（上限为 1000 帧）并进行精确的时间戳对齐，将候选帧嵌入到两个互补的空间中（用于评估问题相关性的 SigLIP 和用于评估语义相似性的 DINOv2），并通过贪婪地最大化模块化相关性项和设施选址覆盖项的加权和来选择帧。该目标函数是归一化的、单调的且子模的，从而提供了标准的 (1-1/e) 贪婪近似保证。为了兼顾与问题相关的相关性和覆盖度之间的权衡，我们引入了四种预设策略和一个轻量级的纯文本问题类型分类器，将每个查询路由到其表现最佳的预设。在 MLVU 上的实验表明，在不同的帧预算下，该方法相比均匀采样和近期强基准模型均有一致的准确率提升，且在预算紧张的情况下提升最为显著。
