...
motivation: 视觉语言模型的错误模式往往难以解释，需要一种交互式工具来分析模型内部视觉概念的处理过程并进行系统性调试。
method: 开发了 VisualScratchpad 界面，利用稀疏自编码器提取视觉特征，并通过文本-图像注意力机制将其与文本标记建立关联。
result: 通过案例研究揭示了模型在跨模态对齐、误导性视觉概念以及隐藏线索利用方面的三种主要失败模式。
conclusion: 该工具通过可视化视觉概念与文本的交互，为理解 VLM 的内部工作机制和提升其可解释性提供了有效手段。
---

## 摘要
高性能的视觉语言模型仍会产生错误答案，且其失败模式往往难以解释。为了提高模型内部机制的可访问性并实现系统化调试，我们引入了 VisualScratchpad，这是一个用于推理过程中视觉概念分析的交互式界面。我们将稀疏自编码器应用于视觉编码器，并通过文本-图像注意力机制将生成的视觉概念与文本标记（tokens）联系起来，从而使我们能够检查哪些视觉概念既被视觉编码器捕获，又被语言模型所利用。VisualScratchpad 还提供了一个标记-潜变量（token-latent）热图视图，为因果分析中的有效概念消融提供了一组充足的潜变量建议。通过案例研究，我们揭示了三种尚未被充分探索的失败模式：有限的跨模态对齐、误导性的视觉概念以及未被使用的隐藏线索。项目主页：https://hyesulim.github.io/visual_scratchpad_projectpage/
