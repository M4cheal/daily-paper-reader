...
tldr: "本研究针对实时循环学习（RTRL）计算成本极高的问题，发现梯度传输中存在巨大冗余。通过仅传播随机选择的极小比例（如6%）路径，即可在RNN、LSTM和Transformer中恢复大部分全量RTRL的性能。这种稀疏传播不仅降低了计算复杂度，还在处理混沌动力学和神经数据漂移时表现出比全量RTRL更好的数值稳定性。研究揭示了雅可比矩阵的近各向同性是其冗余性的根源，为高效在线学习提供了新路径。"
selection_source: fresh_fetch
motivation: 旨在解决实时循环学习（RTRL）因计算复杂度高达 O(n^4) 而难以在大型神经网络中进行在线梯度计算的瓶颈。
method: 提出一种稀疏梯度传输方法，通过仅随机选择并传播极少量的雅可比路径来近似完整的梯度更新。
result: 在多种架构上仅需极低比例的路径传播即可恢复绝大部分性能，且在混沌动力学和神经数据任务中展现出优于全量RTRL的数值稳定性。
conclusion: 研究证明了循环雅可比矩阵具有高度冗余和近各向同性，使得极度稀疏的在线学习在保持性能的同时显著提升了计算效率和鲁棒性。
---
