...
tldr: 针对视频推理中时空定位困难且推理成本高的问题，本文提出 VisionCoach 框架。该框架在强化学习训练阶段，通过视觉提示引导模型关注关键证据并抑制干扰，并利用自蒸馏技术使模型在推理时无需额外提示即可实现精准定位。实验表明，该方法在多个视频理解基准上达到 SOTA 水平，且保持了高效的推理性能。
selection_source: fresh_fetch
motivation: 现有的视频推理模型在时空定位上表现不佳，且往往依赖昂贵的标注数据或推理时感知工具。
method: 提出一种输入自适应的强化学习框架，利用视觉提示选择器和受提示引导的时空推理器，结合物体感知奖励进行优化。
result: 在 V-STAR、VideoMME、VideoMMMU 等多个主流视频推理和理解基准测试中均取得了领先的性能。
conclusion: 证明了在训练阶段引入视觉提示引导并配合自蒸馏，能有效提升模型的时空定位能力且不增加推理负担。
---
